...
sentinel_cfg = HelpforcancerConfig.tokenize(sentinel_config_file)
sentinel_version = "1.2.0"
min_helpforcancerd_proto_version_with_sentinel_ping = 70207


def get_helpforcancer_conf():
    if sys.platform == 'win32':
        helpforcancer_conf = os.path.join(os.getenv('APPDATA'), "hfcc/hfcc.conf")
    else:
        home = os.environ.get('HOME')

        helpforcancer_conf = os.path.join(home, ".hfcc/hfcc.conf")
        if sys.platform == 'darwin':
            helpforcancer_conf = os.path.join(home, "Library/Application Support/hfcc/hfcc.conf")

    helpforcancer_conf = sentinel_cfg.get('helpforcancer_conf', helpforcancer_conf)

    return helpforcancer_conf
...
helpforcancer_conf = get_helpforcancer_conf()
network = get_network()
