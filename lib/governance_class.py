...
class GovernanceClass(object):
    ...
    def vote(self, helpforcancerd, signal, outcome):
        return self.go.vote(helpforcancerd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, helpforcancerd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(helpforcancerd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(helpforcancerd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):

        import helpforcancerlib
        obj_data = helpforcancerlib.SHIM_serialise_for_helpforcancerd(self.serialise())
    ...
    def helpforcancerd_serialise(self):
        import helpforcancerlib
        return helpforcancerlib.SHIM_serialise_for_helpforcancerd(self.serialise())
