"""
helpforcancerd JSONRPC interface
"""
import sys
import os
...
class HelpforcancerDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some helpforcancerd methods
    ...
    @classmethod
    def from_helpforcancer_conf(self, helpforcancer_dot_conf):
        from helpforcancer_config import HelpforcancerConfig
        config_text = HelpforcancerConfig.slurp_config_file(helpforcancer_dot_conf)
        creds = HelpforcancerConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = helpforcancerlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = helpforcancerlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = helpforcancerlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_helpforcancerd_proto_version_with_sentinel_ping)
