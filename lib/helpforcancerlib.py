...
def is_valid_helpforcancer_address(address, network='mainnet'):
    ...
    helpforcancer_version = 65 if network == 'testnet' else 55
    ...
    try:
        ...
        # rescue from exception, not a valid Helpforcancer address
        return False

    if (address_version != helpforcancer_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.helpforcancerd_serialise())
...
def SHIM_serialise_for_helpforcancerd(sentinel_hex):
    from models import GOVOBJ_TYPE_STRINGS

    # unpack
    obj = deserialise(sentinel_hex)

    # shim for helpforcancerd
    govtype_string = GOVOBJ_TYPE_STRINGS[obj['type']]

    # superblock => "trigger" in helpforcancerd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # helpforcancerd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    helpforcancerd_hex = serialise(obj)
    return helpforcancerd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('helpforcancer.conf')
