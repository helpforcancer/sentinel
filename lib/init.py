...
def has_helpforcancer_conf():
    import config
    import io

    valid_helpforcancer_conf = False

    # ensure helpforcancer_conf exists & readable
    #
    # if not, print a message stating that Helpforcancer Core must be installed and
    # configured, including JSONRPC access in helpforcancer.conf
    try:
        f = io.open(config.helpforcancer_conf)
        valid_helpforcancer_conf = True
    except IOError as e:
        print(e)

    return valid_helpforcancer_conf
...
def main():
    ...
    if not has_helpforcancer_conf():
        print("Helpforcancer must be installed and configured, including JSONRPC access in helpforcancer.conf")
        sys.exit(1)
