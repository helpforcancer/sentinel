...
class GovernanceObject(BaseModel):
    ...
    # sync helpforcancerd gobject list with our local relational DB backend
    @classmethod
    def sync(self, helpforcancerd):
        golist = helpforcancerd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_helpforcancerd(helpforcancerd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_helpforcancerd(self, helpforcancerd, rec):
        import decimal
        ...
        # exclude any invalid model data from helpforcancerd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from helpforcancerd, with every run
        ...
        # get/create, then sync payment amounts, etc. from helpforcancerd - Helpforcancerd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(helpforcancerd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from helpforcancerd! %s" % e)
            govobj.vote_delete(helpforcancerd)
            return (govobj, None)
    ...
    def vote_delete(self, helpforcancerd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(helpforcancerd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, helpforcancerd, signal, outcome):
        import helpforcancerlib
        ...
        output = helpforcancerd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = helpforcancerlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(helpforcancerd, signal)

    def sync_network_vote(self, helpforcancerd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = helpforcancerd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = HELPFORCANCERD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 helpforcancer addr, non-multisig
            if not helpforcancerlib.is_valid_helpforcancer_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Helpforcancer address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Helpforcancer Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (helpforcancerlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = HELPFORCANCERD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not helpforcancerlib.is_valid_helpforcancer_address(addr, config.network):
    ...
    def hash(self):
        import helpforcancerlib
        return helpforcancerlib.hashit(self.serialise())
